...
def grille():                           #définition de la grille de départ
    ...
    bombe_case= []
    for i in range((nb_ligne*nb_collone)//8):        #def bombe sur des cases aléatoirement
...
def ecriture_nombre():                      #variable pour écrire le nombre de bombe autour de la case séléctionner
    ...
    if nb_bombe_aut ==0:
        print(collone_list_temp)
        
        collone_list_temp=collone_list_temp
        ligne_list_temp=ligne_list[k]
        collone_list[i]= collone_list[i-1]
        ligne_list[k]= ligne_list[k-1]
        ecriture_nombre()
        print(collone_list_temp)
        collone_list[i]= collone_list_temp
        ligne_list[k]= ligne_list_temp
        print(collone_list_temp)
    tt_g.color("black")
    nombre_bombe_autour[:]= []
    tt_g.penup()
